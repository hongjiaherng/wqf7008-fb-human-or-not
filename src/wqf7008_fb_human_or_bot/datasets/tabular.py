"""Tabular dataset: per-bidder engineered feature matrix (cached to parquet)."""
...
# 1 second of real time ~ 52,631,578 raw time units (from EDA).
...
def build_tabular(
    ...
    """Return (X, bidder_ids, y). Cached to `path_cfg.cache_dir` as parquet."""
...
def feature_cols(X: pl.DataFrame) -> list[str]:
    """Tabular feature column names (everything except the `bidder_id` key)."""
